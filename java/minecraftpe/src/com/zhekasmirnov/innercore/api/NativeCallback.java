...
import com.zhekasmirnov.apparatus.job.Job;
...
public class NativeCallback {
    ...
    private static void showExperimentalWorkbenchWarning() {
        ...
        for (int i = 0; i < 2; i++) {
            HorizonApplication.getTopRunningActivity().runOnUiThread(new Runnable() {
            	@Override
            	public void run() {
    ...
    public static void onLocalServerStarted() {
        ...
        // TODO(ecj): AdaptedScriptAPI.IDRegistry.rebuildNetworkIdMap();
        ...
        // TODO(instant): WorldDataSaverHandler.getInstance().onLevelSelected(worldDirFile);
    ...
    private static void onLevelPostLoaded(boolean isServer) {
        ...
        // TODO(instant): WorldDataSaverHandler.getInstance().onLevelLoading();
    ...
    public static void onGameStopped(boolean isServer) {
        ...
        // TODO(instant): boolean callLegacyCallback = isServer == (Minecraft.getLastWorldState() == Minecraft.GameState.HOST_WORLD);
        boolean callLegacyCallback = true;
    ...
    public static void onLocalTick() {
        ...
        // TODO(instant): if (!NativeAPI.isLocalServerRunning()) {
        ...
        if (!isLocalTickDisabledDueToError) {
            ...
            try {
                ...
                // ecs
                // TODO(ecj): localTickingSystem.run(ECS.getEntityManager());
            } catch (Throwable e) {
        ...
        if (isServerTickDisabledDueToError || isLocalTickDisabledDueToError) {
            ...
            // TODO(ecj): message.append(ChatColor.RED).append(AdaptedScriptAPI.Translation.translate("system.thread_stopped"));
            message.append("\nstopped threads: ");
            if (isLocalTickDisabledDueToError) {
    ...
    public static void onTick() {
        ...
        if (!isServerTickDisabledDueToError) {
            try {
                ...
                InventorySource.tick();
                // TODO(instant): ArmorRegistry.onTick();
                // TODO(instant): WorldDataSaverHandler.getInstance().onTick();
                ...
                // run ecs
                // TODO(ecj): serverTickingSystem.run(ECS.getEntityManager());
            } catch (Throwable e) {
    ...
    public static void onEntityHurt(long entity, long attacker, int damageType, int damageValue, boolean someBool1, boolean someBool2) {
		...
		// TODO(instant): if (entity == NativeAPI.getPlayer()) {
    ...
    public static void onWorkbenchCraft(long containerPtr, final long player, int size) {
        final NativeWorkbenchContainer container = new NativeWorkbenchContainer(containerPtr, size, player);
        ...
        final WorkbenchRecipe recipe = WorkbenchRecipeRegistry.getRecipeFromField(container, "");
        ...
        if (!NativeAPI.isDefaultPrevented() && recipe != null) {
            ...
            if (!recipe.isVanilla()) {
                ...
                Network.getSingleton().getServerThreadJobExecutor().add(new Job() {
                	@Override
                	public void run() {
